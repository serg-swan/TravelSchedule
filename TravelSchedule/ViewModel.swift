...
class TravelViewModel {
    ...
    // MARK: - Подмодель для выбора города и станции
    ...
    private let allStations = MockData.cities.flatMap(\.stations)

    // MARK: - Общая фильтрация городов
    private func filterCities(searchText: String) -> [City] {
        let allCities = MockData.cities
    ...
    private func filterStations(cityName: String, searchText: String) -> [Station] {
        guard let city = MockData.cities.first(where: { $0.name == cityName }) else { return [] }
...
//    // MARK: - Очистка поиска
//    func clearFromSearch() { from.searchText = "" }
//    func clearToSearch() { to.searchText = "" }
}
